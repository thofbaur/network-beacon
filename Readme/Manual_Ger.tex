...
\node[state] 						(V) [below left of=I] 	{V};
  \path[every node/.style={font=\sffamily\small},->]
    (S) edge 					node {Kontakt I, $t_{infect}$} 	(E)
	    edge 	[bend right]	node {Kontakt Z} 					(V)
    (E) edge  					node {$t_{exposedalt}$} 				(I)
...
\begin{itemize}
\item \#define ADV\_INTERVAL				100 // Advertisement interval in milliseconds
...
\end{itemize}
\subsection{Netzwerk}
Die Aufzeichnung des Netzwerks erfolgt über Aufzeichnen der Kontakte die über eine definierbare Zeit gesehen werden.
Es werden nur die bekannten Beacons überwacht, entweder per ID (1 Byte) oder per MAC Adresse (6 Byte). Es wird ein Array vorgehalten, in dem für jedes bekannte Beacon die folgenden Informationen gespeichert werden:
\begin{itemize}
	\item Sekunden seit der Kontakt aktiv ist. 3 Byte
	\item Wurde der Kontakt im letzten Zyklus gesehen. 1 Byte
	\item Ist der Kontakt gerade aktiv. 1 Byte
	\item Wie lange wurde der Kontakt nicht mehr gesehen (Timeout). 1 Byte
\end{itemize}
Erreicht ein Kontakt das Limit für den Timeout und bestand der Kontakt länger als das definierte Limit, wird der Kontakt in den Datenspeicher geschrieben. Der Datenspeicher ist ein Ringspeicher mit folgenden Informationen:
\Begin{itemize}
	\item
\end{itemize}
...
%(= \verb!status_infect + status_batt*16+inf_rev*32!),
...
\subsection{Setup 2026}
Visual Studio Code 1.119.0
Entpacken des SDK 12.3 in einen bekannten Ordner und anpassen des Pfades in allen Makefiles auf diesen Ordner.
nrf-command-line-tools-10.24.2-x64
\end{document}
